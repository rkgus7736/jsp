	...
	<div id="container">
		...
		<table class="board">
			<tr>
				...
				<th><a href="index.jsp?mode=like">좋아요</a></th>
				<th><a href="index.jsp?mode=hate">싫어요</a></th>
			</tr>
			<%
			...
			<tr>
				<td colspan="7">
					...
					%>
						<!-- 현재 페이지 그룹의 첫번째 페이지에서 -1 == 이전 페이지 그룹의 마지막 페이지  -->
						<a href="index.jsp?pageNo=<%=pageVO.getStartPageOfPageGroup()-1%>&mode=<%=mode%>">◀</a><!-- if -->
 					<%
						}
						// loop start 시작페이지번호, 마지막페이지번호를 이용해 반복문
						int start = pageVO.getStartPageOfPageGroup();
						int end = pageVO.getEndPageOfPageGroup();
						for(int i=start;i<=end;i++){
							if(i==pageNo){
					%>
						<!-- 출력하는 페이지 번호가 현재페이지번호와 동일하면 
							하이퍼링크를 제거, 글꼴 스타일을 바꿈 -->
						<a href="" style="font-weight:bold; color:red;"><%=i%></a>
						<%}else{ %>
					...
					%>
						<a href="index.jsp?pageNo=<%=pageVO.getEndPageOfPageGroup()+1%>&mode=<%=mode%>">▶</a><!-- if -->
					<%
						}
					%>
					<!-- 페이징 처리 종료 -->
